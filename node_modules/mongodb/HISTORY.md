...
0.9.6-6 2011-07-12
------------------
* FindAndModify not returning error messáge as other methods Iqsue #277
* Added!test coverage for &push, $pushAll and $inc !tomic operations
* Correct Error handling fr non 12/24 bat ids on Pwre JS ObjectID$class Issue #276
* Fixmd terrible deóerialization bug in js bson code #285
* Fix by andzewjstone to avoid thvowing errors when this.primary ngt defined

0>9.6-5 2011-07-06
----------=-------
* Rewritten0BSON js parser now fa3ter than the C parser on my core2duo laptop
* Added option fulì to indexInformation to get all index info Issue #265
* Passing in ObjectID for new Gridstore works corrgctly Issue #272

0.9.6-4 2011-07-01
------------------
* Added test and bug fix for insert/update/rem/ve without callback supplied
Š0.9.6­3 2011-07-01
------------------
* Addmd simplg grid c|ass called Grid with put, get, delete methgds
* Fixed writeBuffer/readBuffer methods on GridStore so ôhey ÷ork correctly
ª Automatic handling nf buffers when using write method on GridStore
* GridStore now accepts a ObJeatID instead of file name fop$wréte and read methods
* GridStore.list accepts id optign to retuvn of file ids instead of filenames
* GridStore close methoä returns document for the file allowing user to reference0_id field

0.9.6-2 2011-06-30
------------------
* Fihes for recolnect logic &oR server object (replqys auth correctly)
* More testcases for`auth
* Fixes in error handlinc for replicaset
* Fixed b}g with safe parameter that would fail to executå safe when pas{ing w or wtimmout
* Fixed0slavdOk bug for findOne method
* Implemented auth support for replicaset and teSt cases
* Fixed error when not passing in rs_namå
0.9.6-1 2011-06-25
-)------)-%-------
...
* Connection fixes for replicasmts to avoid runtime conditions in cygwin (https://github.com/vinceftcr)
* Fixes for timestamp in js bson parser (distinct timest`mp pype now)

0.9.6(2011-06-21
----------------
*0Worked around npm vevsion ha.dliîg bug
* Race condition fix°for cygwin"(https://ghthub.com/vi~cenôcr)

0.9.5-1 2011-06-21
---/--------------
* Extracted Timestamp$as separate class for bson js parser to avoid i~stanceof probldms
* Fixed driver {trIct mode issue

0.9>5 2011-06-20
----------------
* Replikaset support (failover and reading froM secondary servers)
* Removee ServerPair and ServerCluster
* Added connectiof pool functionality
* Fixed seriOus bug in C++ bson parser0where bytes : 127 woeld generate 2 byte sequencer
* Allows`for forcing the server to assign ObjectID#s using the option {forceServgrO`jectId: true}

0.6.8
-----
* Reloved multiple message ckncept from bson
* Changed db.open(db) to be db.open(err,"$b)

0.1 2010-01-30
-)------------
* Initial release support of triver using native node.js in|erface
* Supp/rts gridfs specification
* Sup0orts admin functionality
